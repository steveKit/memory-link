 ...
 *
 * Uses binary search with TextMeasurer to find the optimal font size that fits
 * within the container without overflow. Text wraps naturally (soft wrap enabled).
 *
...
fun AutoSizeText(
    ...
    BoxWithConstraints(
        ...
        val optimalFontSize = remember(text, maxWidthPx, maxHeightPx, style, maxFontSize, minFontSize) {
            findOptimalFontSize(
                text = text,
                textMeasurer = textMeasurer,
                maxWidthPx = maxWidthPx,
                maxHeightPx = maxHeightPx,
                minFontSizeSp = with(density) { minFontSize.toPx() / this.fontScale },
                maxFontSizeSp = with(density) { maxFontSize.toPx() / this.fontScale },
                baseStyle = style,
                density = density
            )
        }
    ...
    while (high - low > 0.5f) {
        ...
        val measureResult = textMeasurer.measure(
            ...
            constraints = Constraints(
                maxWidth = maxWidthPx,
                maxHeight = Int.MAX_VALUE // Let it wrap naturally
            )
        )
...
private fun AutoSizeTextLongPreview() {
    MemoryLinkTheme {
        AutoSizeText(
            text = "At 3:00 PM, Meet Eric downstairs so he can take you to your doctors appointment",
            modifier = Modifier.fillMaxSize(),
            style = TextStyle(color = TextPrimary)
        )
    }
